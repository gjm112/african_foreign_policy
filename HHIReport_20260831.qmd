...
This analysis seeks to evaluate the relationship between an African country's Herfindahl-Hirschman Index (HHI) and their democracy score. The HHI measures the share of an African country's official development assistance gross (ODAG) share from external countries. We used democracy scores from Freedom House (TOTLIB1), the POLITY IV database (POLITY2), and the Liberal Democracy Index from the V-Dem Institute (V2x_Libdem).

This main analyses were split into a reduced and full model. The reduced model only considers democracy as a fixed effect, while including random intercepts for the African country and year. These random effects account for correlation between observations within the data structure.

The full model includes all of the parts of the reduced model while adding the African country's population, gross national income (GNI), amd total ODAG (Note: When ODAG is not observed it is set to 0 in the analysis). We applied a log transformation to each of these predictor variables due to their fairly extreme right skew and quadratic or cubic transformations when necessary to capture nonlinear relationships. We applied this model framework to lagged democracy scores of 1, 2, and 3 years.

In our previous analysis, we included additional models for the POLITY democracy score from the POLITY IV database. Here, the POLITY model was dropped due to convergence issues in parameter estimation with the addition of ODAG, which both have a large number of zeros. These convergence issues were not present when using POLITY2.

In the reduced models, every V2x_Libdem and POLITY2 coefficient (besides POLITY2 in the lag0 model) is significant at $\alpha$ = 0.05. The negative direction of their coefficients indicate that, as an African country is more democratic, it tends to have more countries providing development aid and vice versa.

In the full models, every coefficient besides POLITY2 in the lag0 and lag1 models is significant at $\alpha$ = 0.05. The negative direction of the V2x_Libdem and POLITY2 coefficients and positive direction of the TOTLIB1 coefficients provide the same conclusion as for the reduced models. To reiterate, as an African country is more democratic, it tends to have more countries providing development aid and vice versa.

# Reduced Model

The reduced model equations are given below, where the response $Y_{it}$ ($Y$ being HHI) ranges from 1/$k$, $k$ being the number of possible sources of ODAG, to 1 for African country $i$ in year $t$. We applied a log transformation to HHI to fix its non-constant variance, where the residuals increased in size as the fitted values increased. The predictor $X_{1it}$ represents the democracy score of African country $i$ in year $t$.
...
# HHI Model Information and Relevant Visualizations
