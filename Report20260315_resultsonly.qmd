...
Three different variables of democracy (each with different inclusive dates) to use separately in the analyses as the independent variable:
 ...
 3.	Freedom House democracy variable (1972 - 2024)
    Variable name in the dataset: TOTLIB. It ranges from 1 (best) to 14 (worst). It is a combination of the following two variables, which people sometimes run separately: Civil liberties - Variable name in the dataset: 
    -CIVLIB. It ranges from 1 (best) to 7 (worst).     -Political rights: Variable name in the dataset: POLLIB. It ranges from 1 (best) to 7 (worst).
...
## CONTROL VARIABLES RELATED FOR AFRICAN COUNTRIES (AFRICAN FACTORS)
 ...
 - Region of Africa (1= North Africa, 2 = East Africa, 3 = Southern Africa, 4= Central Africa, 5 = West Africa)
 - African Colonial Past (Colpast2) (1 = French, 2 = British, 3 = Portuguese, 4 = Spanish, 5 = Italian, 6 = Belgian, 7 = German, 8 = United States, 9 = Not colony, 10 = unique)
  - African Ideology (1 = capitalist, 2 = socialist, 3 = Marxist)
 - African Historical Period
  - Entire independence era
...
mod_DEMOC <- glmer(N_EMBASSY ~ DEMOC + logGNI + logPOPULATN  + REGION + COL2 + factor(IDEOLOGY) + factor(AFR_HIST_PERIOD) + (1|COUNTRY)+ (1|YEAR), data = modelset, family = "poisson", control = glmerControl(optimizer = "bobyqa"))
mod_DEMOC_lag1 <- glmer(N_EMBASSY ~ DEMOC_lag1 + logGNI + logPOPULATN  + REGION + COL2 + factor(IDEOLOGY) + factor(AFR_HIST_PERIOD) + (1|COUNTRY)+ (1|YEAR), data = modelset, family = "poisson", control = glmerControl(optimizer = "bobyqa"))
mod_DEMOC_lag2 <- glmer(N_EMBASSY ~ DEMOC_lag2 + logGNI + logPOPULATN  + REGION + COL2 + factor(IDEOLOGY) + factor(AFR_HIST_PERIOD) + (1|COUNTRY)+ (1|YEAR), data = modelset, family = "poisson", control = glmerControl(optimizer = "bobyqa"))
mod_DEMOC_lag3 <- glmer(N_EMBASSY ~ DEMOC_lag3 + logGNI + logPOPULATN  + REGION + COL2 + factor(IDEOLOGY) + factor(AFR_HIST_PERIOD) + (1|COUNTRY)+ (1|YEAR), data = modelset, family = "poisson", control = glmerControl(optimizer = "bobyqa"))
