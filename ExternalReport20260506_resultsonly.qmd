...
Our external analysis seeks to evaluate the relationship between an external country's democracy score and the likelihood of an embassy in an internal country in a given year. With the exception of official development assistance (ODA) aid, almost all modeled characteristics are independent of internal African countries. We used democracy scores from Freedom House (TOTLIB1) and the POLITY IV database (POLITY and POLITY2).
...
- Missing data stuff.  How many of each columns are missing?   
- Add in density for the predictors on log scale.  
- Summary for all varaibles in the models.  
